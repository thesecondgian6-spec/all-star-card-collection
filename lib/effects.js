...
/* ---------------- fly-to-wallet coin particle ---------------- */

/** Animates a small coin icon from originEl's position to the wallet display. Purely cosmetic. */
export function flyCoinToWallet(originEl, count = 3) {
  if (typeof document === 'undefined' || !originEl) return;
  const walletEl = document.querySelector('.wallet-item.coins');
  if (!walletEl) return;
  const from = originEl.getBoundingClientRect();
  const to = walletEl.getBoundingClientRect();

  for (let i = 0; i < count; i++) {
    setTimeout(() => {
      const coin = document.createElement('div');
      coin.textContent = '🪙';
      const startX = from.left + from.width / 2 + (Math.random() - 0.5) * 20;
      const startY = from.top + from.height / 2 + (Math.random() - 0.5) * 20;
      coin.style.cssText = `position:fixed;left:${startX}px;top:${startY}px;font-size:18px;z-index:250;pointer-events:none;transition:transform .55s cubic-bezier(.2,.8,.2,1), opacity .55s ease;`;
