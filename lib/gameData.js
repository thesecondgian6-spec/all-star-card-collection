 ...
 * with the client.
 */
export function estimatePending({ earnRate, level, lastTick, capHours, capacityBonusHours = 0, multiplier = 1 }) {
  if (!level) return 0;
  ...
  return effectiveSec * Number(earnRate) * Number(level) * Number(multiplier);
}

/** Duplicate copies + coins needed to go from currentLevel to currentLevel+1. */
export function levelUpCost(card, currentLevel) {
  const dupesNeeded = currentLevel;
  const coinCost = Math.ceil(Number(card.earn_rate) * 500 * currentLevel);
  return { dupesNeeded, coinCost };
}

export function upgradeCost(upgrade, currentLevel) {
  return Math.ceil(Number(upgrade.base_cost) * Math.pow(Number(upgrade.cost_growth), currentLevel));
}
