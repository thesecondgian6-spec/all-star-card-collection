...
export default function UpgradesPage() {
  ...
  return (
    <AppShell coins={playerState?.coins} gems={playerState?.gems}>
      <div className="panel">
        <h2>Upgrades</h2>
        <p className="sub">Spend gems (earned from achievements and daily logins) on permanent upgrades.</p>
              ...
              <div className="upgrade-card" key={u.id} style={{ opacity: locked ? 0.55 : 1 }}>
                ...
                {locked ? (
                  ...
                  <button className="btn full" disabled={!affordable || busy === u.id} onClick={() => buy(u)}>
                    {busy === u.id ? 'Purchasing…' : `💎 ${fmt(cost)} — Upgrade`}
                  </button>
                )}
              </div>
            );
          })}
          {upgrades.length === 0 && <div className="empty">No upgrades configured yet.</div>}
        </div>
      </div>
